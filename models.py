...
# --- DIAGNOSTIC PRINT 1 --- 
print(f"[DEBUG PRE-DOTENV] os.environ.get('SUPABASE_PORT'): '{os.environ.get('SUPABASE_PORT')}'", file=sys.stderr)
# --- END DIAGNOSTIC PRINT 1 ---

load_dotenv() # Load .env file for local development

# --- DIAGNOSTIC PRINT 2 --- 
print(f"[DEBUG POST-DOTENV] os.environ.get('SUPABASE_PORT'): '{os.environ.get('SUPABASE_PORT')}'", file=sys.stderr)
...
if DB_CONNECTION_TYPE == 'SUPABASE':
    ...
    # --- DIAGNOSTIC PRINT 3 --- 
    print(f"[DEBUG IN-SUPABASE-IF] SUPABASE_PORT variable value: '{SUPABASE_PORT}'", file=sys.stderr)
    # --- END DIAGNOSTIC PRINT 3 ---

    if not all([SUPABASE_HOST, SUPABASE_RAW_PASSWORD]):
        print("CRITICAL WARNING: DB_CONNECTION_TYPE is SUPABASE, but SUPABASE_HOST or SUPABASE_PASSWORD is not set. Falling back to SQLite.", file=sys.stderr)
        ...
        try:
            ...
            print(f"[INFO] Connecting to Supabase PostgreSQL database: {db_url_obj.render_as_string(hide_password=True)}", file=sys.stderr)
        except ValueError as e:
            print(f"[ERROR] Failed to convert SUPABASE_PORT to int. Value was: '{SUPABASE_PORT}'. Error: {e}", file=sys.stderr)
            print("CRITICAL WARNING: Falling back to SQLite due to SUPABASE_PORT conversion error.", file=sys.stderr)
            DATABASE_URL_STR = 'sqlite:///dread_system.db'
elif DB_CONNECTION_TYPE == 'SQLITE':
    DATABASE_URL_STR = 'sqlite:///dread_system.db'
    print(f"[INFO] Using SQLite database: {DATABASE_URL_STR}", file=sys.stderr)
else:
    print(f"CRITICAL WARNING: Unknown DB_CONNECTION_TYPE '{DB_CONNECTION_TYPE}'. Falling back to SQLite.", file=sys.stderr)
    DATABASE_URL_STR = 'sqlite:///dread_system.db'
